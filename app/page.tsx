      ...
      <div className="relative z-10 max-w-7xl mx-auto flex min-h-[420px] md:min-h-140">
        ...
        <div className="w-full md:w-[46%] shrink-0 flex flex-col justify-center py-10 md:py-16 px-6 md:px-8 lg:px-16">
          ...
          <div className="flex items-center gap-3 sm:gap-5 md:gap-6 mb-8 md:mb-10 flex-wrap">
            {heroIcons.map((item) => (
              <div key={item.label} className="flex flex-col items-center gap-2">
                <div className="w-11 h-11 md:w-13 md:h-13 rounded-full border border-[#aaa] flex items-center justify-center hover:border-[#1B2B4B] transition-colors bg-white/30 backdrop-blur-sm">
                  {item.icon}
                </div>
          ...
          <div>
            <Link
              href="/booking"
              className="inline-flex items-center gap-2 bg-[#1B2B4B] text-white text-[13px] md:text-[14px] font-medium px-6 md:px-7 py-3 md:py-3.5 hover:bg-[#152240] transition-colors"
            >
              상담 예약하기 &nbsp;→
            </Link>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4">
          {serviceNavItems.map((item, i) => {
            ...
            return (
              ...
              >
                <div className="w-11 h-11 md:w-14 md:h-14 rounded-full border border-[#e8e8e8] flex items-center justify-center shrink-0 group-hover:border-[#C9A84C] transition-colors">
                  {item.icon}
                </div>
...
function Gallery() {
  return (
    <section className="w-full bg-white border-t py-6 md:py-16 border-[#e8e8e8]">
      <div className="max-w-[1280px] mx-auto px-3 md:px-6 flex flex-col lg:flex-row gap-4 md:gap-6">
        {/* 3 image columns */}
        <div className="flex flex-row flex-nowrap w-full gap-1.5 md:gap-2">
          {galleryItems.map((item, i) => (
            <div key={i} className="flex-1 min-w-0 flex flex-col border shadow-sm md:shadow rounded-xl md:rounded-2xl border-[#e8e8e8]">
              {/* Image */}
              <div className="relative w-full rounded-t-xl md:rounded-t-2xl overflow-hidden aspect-[4/3] md:aspect-square">
                <Image src={item.src} alt={item.title} fill sizes="(max-width: 768px) 30vw, 320px" className="object-cover" />
              </div>
              {/* Caption */}
              <div className="px-1.5 py-2 md:px-3 md:py-5 flex-1 flex flex-col text-center">
                <h4 className="text-[9px] sm:text-[11px] md:text-[14.5px] font-bold text-[#1B2B4B] mb-0.5 md:mb-2 leading-snug break-keep" style={{ fontFamily: "'Noto Serif KR', serif" }}>
                  {item.title}
                </h4>
                <p className="hidden md:block mt-auto text-[12.5px] text-[#666] leading-relaxed whitespace-pre-line break-keep">{item.desc}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        {/* Navy info card */}
        <div className="w-full lg:w-70 shrink-0 rounded-2xl bg-[#1B2B4B] px-5 py-6 md:px-7 md:py-10 flex flex-col justify-center">
          <h3 className="text-[15px] md:text-[19px] font-bold text-white mb-2 md:mb-4" style={{ fontFamily: "'Noto Serif KR', serif" }}>
            청담사주작명원
          </h3>
          <p className="hidden md:block text-[13px] text-white/70 leading-relaxed mb-7">단순한 운세 풀이가 아닌, 사주의 구조와 흐름을 바탕으로 현실적인 방향을 제시하는 상담을 지향합니다.</p>
          <ul className="flex flex-col gap-2 md:gap-3 mb-5 md:mb-8">
            {infoList.map((item) => (
              <li key={item} className="flex items-start gap-2">
                ...
                <span className="text-[11px] md:text-[12.5px] text-white/80 leading-relaxed">{item}</span>
              </li>
            ))}
          </ul>
          <Link
